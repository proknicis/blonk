...
const initialKeys: ApiKey[] = [
    { id: "k1", label: "Microsoft 365", service: "Microsoft", key: "sk-ms-••••••••••••••••3F2A", added: "2026-03-12" },
    { id: "k2", label: "OpenAI GPT-4", service: "OpenAI", key: "sk-••••••••••••••••••••••7B1C", added: "2026-04-01" },
];

export default function SovereigntyPage() {
    const [residency, setResidency] = useState<"eu-cloud" | "local">("eu-cloud");
    ...
    const handleAddKey = () => {
        ...
        setKeys(prev => [...prev, {
            id: `k${Date.now()}`, label: newLabel, service: newService || "Custom",
            key: newKey, added: new Date().toISOString().split("T")[0]
        }]);
                ...
                <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 24, padding: 32 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {[
                            ...
                            <button
                                key={opt.id}
                                onClick={() => setResidency(opt.id as typeof residency)}
                                style={{
                            ...
                            <tr key={k.id} style={{ borderBottom: i < keys.length - 1 ? "1px solid var(--border)" : "none" }}>
                                ...
                                <td style={{ padding: "14px 20px" }}>
                                    <div style={{ display: "flex", gap: 6 }}>
                                        ...
                                        <button onClick={() => setKeys(prev => prev.filter(x => x.id !== k.id))} style={{ padding: "6px 10px", background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, cursor: "pointer", color: "#ef4444", display: "flex", alignItems: "center" }}>
                                            <Trash2 size={13} />
                                        </button>
                                    </div>
                                </td>
