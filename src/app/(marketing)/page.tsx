            ...
            <div className={styles.mainContent}>
                    ...
                    <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 32 }}>
                                ...
                                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
                                    {['Core Governance Engine', 'Public Sovereign Cloud', 'Standard Audit Support', '5 Seat Integrated Vault'].map(f => (
                                        <li key={f} style={{ display: 'flex', alignItems: 'center', gap: 12, fontWeight: 700, fontSize: '0.95rem' }}>
                                            <div style={{ width: 18, height: 18, background: '#34D186', borderRadius: '50%', color: '#FAFAFA', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem' }}>✓</div>
                                            {f}
                                        </li>
                                    ))}
                                </ul>
                                <Link href="/login" style={{ background: '#111', color: '#FAFAFA', padding: '20px 32px', borderRadius: '100px', textAlign: 'center', textDecoration: 'none', fontWeight: 800, marginTop: 'auto' }}>Initialize BLONK.</Link>
                            </div>
                            ...
                            <div style={{ background: '#FAFAFA', padding: 56, borderRadius: 48, textAlign: 'left', color: '#111', position: 'relative', display: 'flex', flexDirection: 'column', gap: 32, transform: 'translateY(-12px)', border: '1px solid #f0f0f0', boxShadow: '0 40px 80px rgba(0,0,0,0.05)' }}>
                                <div style={{ position: 'absolute', top: 32, right: 32, background: '#34D186', color: '#FAFAFA', fontSize: '0.7rem', fontWeight: 900, padding: '4px 12px', borderRadius: 6 }}>ELITE</div>
                                <div>
                                    <div style={{ fontSize: '0.9rem', fontWeight: 900, textTransform: 'uppercase', color: '#999', marginBottom: 8 }}>Enterprise</div>
                                    <div style={{ fontSize: '3.5rem', fontWeight: 950, letterSpacing: '-0.06em' }}>Custom<span style={{ fontSize: '1rem', color: '#666' }}>/usage</span></div>
                                </div>
                                <p style={{ fontSize: '1.05rem', color: '#555', lineHeight: 1.6 }}>The ultimate administrative infrastructure for Fortune 500 legal and accounting practices. Unlimited vertical scaling across any stack.</p>
                                <div style={{ height: 1, background: '#f0f0f0' }} />
                                ...
                                <Link href="/login" style={{ background: '#111', color: '#FAFAFA', padding: '20px 32px', borderRadius: '100px', textAlign: 'center', textDecoration: 'none', fontWeight: 800, marginTop: 'auto' }}>Inquire for Enterprise.</Link>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section style={{ padding: '160px 24px', textAlign: 'center', background: '#FAFAFA', color: '#111' }}>
                    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
                        ...
                        <Link href="/login" style={{ display: 'inline-flex', alignItems: 'center', gap: 16, background: '#111', color: '#FAFAFA', padding: '24px 64px', borderRadius: '100px', fontSize: '1.25rem', fontWeight: 900, textDecoration: 'none', transition: 'transform 0.3s' }}>
                            Scale your business.
