        ...
        <div className={styles.container}>
            ...
            <div className={styles.card_grid}>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
                        {['Core Governance Engine', 'Public Sovereign Cloud', 'SOC-2 Ready Audit', '5 Seat Integrated Vault'].map(f => (
                            <li key={f} style={{ display: 'flex', alignItems: 'center', gap: 12, fontWeight: 700, fontSize: '1rem' }}>
                                <div style={{ width: 18, height: 18, background: '#34D186', borderRadius: '50%', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem' }}>✓</div>
                                {f}
                            </li>
                        ))}
                    </ul>
                    <Link href="/setup" style={{ background: '#000', color: '#fff', padding: '20px 32px', borderRadius: '100px', textAlign: 'center', textDecoration: 'none', fontWeight: 900, marginTop: 'auto' }}>Select Institutional.</Link>
                </div>
                ...
                <div style={{ background: '#000', padding: 56, borderRadius: 48, textAlign: 'left', color: '#fff', position: 'relative', display: 'flex', flexDirection: 'column', gap: 32, transform: 'translateY(-12px)', boxShadow: '0 40px 80px rgba(0,0,0,0.15)' }}>
                    <div style={{ position: 'absolute', top: 32, right: 32, background: '#34D186', color: '#fff', fontSize: '0.75rem', fontWeight: 900, padding: '6px 16px', borderRadius: 6 }}>ENTERPRISE</div>
                    <div>
                        <div style={{ fontSize: '0.9rem', fontWeight: 900, textTransform: 'uppercase', color: '#666', marginBottom: 8 }}>Infrastructure</div>
                        <div style={{ fontSize: '4rem', fontWeight: 950, letterSpacing: '-0.06em' }}>Inquire</div>
                    </div>
                    <p style={{ fontSize: '1.1rem', color: '#999', lineHeight: 1.6 }}>The absolute operating layer for Fortune 500 legal and accounting firms.</p>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 16, color: '#999' }}>
                        {['Private Sovereign Backbone', 'Unlimited Capacity scaling', 'Strategic Compliance Sync', 'Dedicated Key Manager'].map(f => (
                            <li key={f} style={{ display: 'flex', alignItems: 'center', gap: 12, fontWeight: 700, fontSize: '1rem' }}>
                                <div style={{ width: 18, height: 18, background: '#34D186', borderRadius: '50%', color: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem' }}>✓</div>
                                {f}
                            </li>
                        ))}
                    </ul>
                    <Link href="/setup" style={{ background: '#fff', color: '#000', padding: '20px 32px', borderRadius: '100px', textAlign: 'center', textDecoration: 'none', fontWeight: 900, marginTop: 'auto' }}>Contact Architecture.</Link>
                </div>
            </div>
        </div>
    );
